...
from openedx_events_2_n8n.handlers import (
...
class RegistrationCompletedReceiverTest(TestCase):
    """
    Tests the registration receiver sends the correct information to n8n.
    """
    ...
    @override_settings(N8N_REGISTRATION_WEBHOOK="https://webhook.site")
    ...
    def test_receiver_called_after_event(self, task_mock):
        ...
        self.assertLessEqual(
            expected_payload["user"].items(),
            task_mock.delay.call_args[0][1]["user"].items(),
        )
        self.assertEqual(
...
class EnrollmentCreatedReceiverTest(TestCase):
    ...
    @override_settings(N8N_ENROLLMENT_WEBHOOK="https://webhook.site")
    @patch("openedx_events_2_n8n.handlers.send_data_to_n8n")
    def test_receiver_called_after_event(self, task_mock):
        """
        Test that send_user_data_to_webhook is called the correct information after sending
...
class PersistentGradeEventsTest(TestCase):
    ...
    @override_settings(N8N_PERSISTENT_GRADE_COURSE_WEBHOOK="https://webhook.site")
    @patch("openedx_events_2_n8n.handlers.send_data_to_n8n")
    def test_receiver_called_after_event(self, task_mock):
        """
        Test that send_persistent_grade_course_data_to_webhook is called the correct information after sending
